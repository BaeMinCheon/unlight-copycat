...
static TCHAR * menuText(TCHAR * text, TCHAR choice);
...
Wostream &Wostream::operator << (TCHAR *s)
{
  winptr->print(s);
  return *this;
}
...
void Window::drawBitmap(Bitmap image, int x, int y, int width, int height, float scale)
{
	...
	StretchBlt(hdc, x, y, static_cast<int>((width + 1)*scale), static_cast<int>((height + 1)*scale), hmemdc, 0, 0, width + 1, height + 1, SRCCOPY);
	SelectObject(hmemdc, holdbmp);
	DeleteDC(hmemdc);
	ReleaseDC(hwnd, hdc);
}
...
Application::Application(TCHAR *aTitle,
                         int wdth,
                         int hght,
                         int x,
                         int y) : Window(aTitle,
                                         ...
                                         y)
{
  static TCHAR *aName = TEXT("LAFAPP");
...
int Application::okBox(TCHAR *msg, TCHAR *caption)
{
...
int Application::okBox(const char *msg, TCHAR *caption)
{
	int saveTimer = timerInterval;
	int result;
	...
	TCHAR tmsg[50];
	_tcscpy_s(tmsg, CA2T(msg));
	result = MessageBox(hwnd, tmsg, caption, MB_OK | MB_TASKMODAL);
...
ChildWindow::ChildWindow(TCHAR *aTitle,
                         int wdth,
                         int hght,
                         int x,
                         int y) : Window(aTitle,
                                         ...
                                         y)
{
  static TCHAR *aName = TEXT("LAFCHILD");
  int i;
...
Menu::Menu(Window &aWindow,
           TCHAR *text,
           TCHAR choice) : theWindow(aWindow)
{
  itemCount   = 0;
...
TCHAR *ListBox::add(TCHAR *str)
{
  SendMessage(hwnd, LB_ADDSTRING, 0, (LPARAM) str);
  return str;
}

TCHAR *ListBox::add(const char *str)
{
	static TCHAR tstr[50];
	_tcscpy_s(tstr, CA2T(str));
	SendMessage(hwnd, LB_ADDSTRING, 0, (LPARAM)tstr);
	return tstr;
}
