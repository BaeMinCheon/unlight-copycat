...
public:
  // constructor
  Window(TCHAR *title, int width, int height, int x, int y);
...
public:
  ...
  // non-event behavior
  virtual int  okBox(TCHAR *, TCHAR * = 0);
  virtual int Application::okBox(const char *msg, TCHAR *caption);
...
public:
  // constructors
  Menu(Window &aWindow, TCHAR *text, TCHAR choice = '\0');
...
public:
  // constructors
  BasicMenuItem(Window &aWindow, TCHAR *text, TCHAR choice,
                Menu &aMenu);
...
{
public:
  MenuItem(T &anApp, TCHAR *text, TCHAR choice, Menu &aMenu)
...
public:
  // constructors
  BasicButton(Window &aWindow, TCHAR *aTitle = TEXT("Button"),
